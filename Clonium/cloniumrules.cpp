...
cloniumrules::cloniumrules(QWidget *parent)
{
    ...
    m_lblText = new QLabel("Le plateau de jeu est un échiquier qui carré qui comporte entre 6 et 10 cases de large, en fonction de la durée de la partie souhaitée. Chaque équipe possède des pions de sa couleur. Un pion peut être de niveau 1, 2, 3 ou 4. Il ne peut pas se déplacer, mais on peut augmenter son niveau. Un tour de jeu consiste donc à augmenter le niveau d'un de ses pions. Une fois le niveau 3 atteint, une augmentation de son niveau au niveau 4 entraînera immédiatement une scission en 4 pions de niveau 1 qui se placeront dans les cases adjacentes, latéralement et verticalement (pas en diagonale). Si des pions sont présents dans les cases en question : - S'ils appartiennent à une équipe adverse on les capture, - Dans tous les cas, on augmente leur niveau de 1. Ceci peut entraîner des réactions en chaîne. Lorsque plusieurs pions déclenchent une réaction en chaîne, il peut y avoir plusieurs cases qui sont activées simultanément. On adoptera donc une gestion « tour par tour » des réactions en chaîne.", this);
    m_lblText->setWordWrap(true);
    m_lblText->setFont(QFont("Commic Sans MS", 8));
